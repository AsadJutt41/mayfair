...
<div class="card">
    ...
    <div class="card-body">
            <form  action="{{route('role-permission.update',$roldid)}}" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                <div class="col-md-6 mt-3">
                    <div class="form-group ">
                        ...
                        <select class="form-control select2 custom-select" data-placeholder="Choose one" name="role_id">
                            ...
                            <option value="{{$role->id}}" {{$permissiondata->name==$role->name ? 'selected' : ''}}>{{$role->name}}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6 mt-3">
                    <div class="form-group">
                        ...
                        <select class="form-control select2" data-placeholder="Choose permission" multiple name="permission_id[]">
                            ...
                            <option value="{{$permission->id}}"   @foreach($permissiondata->permissions as $perm){{$perm->id == $permission->id ? 'selected': ''}}   @endforeach>{{$permission->name}}</option>
                            @endforeach
                        </select>
